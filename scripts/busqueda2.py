...
class BlackCanDetector:
    def __init__(self):
        ...
        self.barredora_pub = rospy.Publisher('/barredora', Bool, queue_size=1)
        ...
        self.angular_speed = 2.0 # Adjust for desired rotation speed
        self.linear_speed = 1.1 # Adjust for desired forward speed
    ...
    def image_callback(self, msg):
        ...
        twist = Twist()
        barredora_msg = Bool()
        barredora_msg.data = False # Default to False
            ...
            if cv2.contourArea(largest_contour) > self.detection_threshold_area:
                ...
                if M["m00"] != 0:
                    ...
                    if position == 1:
                        ...
                        barredora_msg.data = True # Publish True when centered
                    # Draw the contour and center (comment out if not needed)
                    cv2.drawContours(visualization_image, [largest_contour], 0, (0, 255, 0), 2)
                    cv2.circle(visualization_image, (int(center_x), int(M["m10"] / M["m00"])), 5, (0, 0, 255), -1)
                else:
        ...
        self.barredora_pub.publish(barredora_msg)
